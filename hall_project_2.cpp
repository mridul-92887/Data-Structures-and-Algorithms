...
void assignStudentToSeat(string buildingName, string roomNumber, string seatNumber) {
    struct Hall* hallPtr = hallList;
    while (hallPtr != nullptr) {
        struct Building* buildingPtr = hallPtr->buildings;
        while (buildingPtr != nullptr && buildingPtr->buildingName != buildingName) {
                    ...
                    while (seatPtr != nullptr) {
                        if (seatPtr->seatNumber == seatNumber) {
                            if (seatPtr->isOccupied) {
                                ...
                                cout << "Student assigned to Seat " << seatNumber << " in Room " << roomNumber << "!" << endl;
                                return;
                            }
                        }
                ...
                while (roomPtr != nullptr) {
                    ...
                    while (seatPtr != nullptr) {
                        file << "        Seat: " << seatPtr->seatNumber
                             << " (Occupied: " << (seatPtr->isOccupied ? "Yes" : "No") << ")" << endl;
                        seatPtr = seatPtr->next;
                    }
                    roomPtr = roomPtr->next;
                }
...
int main() {
    ...
    while (choice != 7) {
        ...
        switch (choice) {
            ...
            case 5: {
                string buildingName, roomNumber, seatNumber;
                cout << "Enter Building Name: ";
                cin >> buildingName;
                ...
                cin >> seatNumber;
                assignStudentToSeat(buildingName, roomNumber, seatNumber);
                break;
            }
            case 6: {
                string filename;
